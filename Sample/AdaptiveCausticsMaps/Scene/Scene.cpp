	...
	verbose(true), sceneFileDataAccessed(false)
{
	...
	lightIntensityModifier = new float( 1.0 );
	extinctionCoef		   = new float( 0.0 );
...
Scene::Scene( char *filename, bool verbose ) : 
	...
	sceneFileDataAccessed(false), lightIntensityModifier(0), extinctionCoef(0), frameTime(0)
{
	// HACK!
	scene = this;
	// END HACK!
	...
	lightIntensityModifier = this->GetSceneFloatVar( "lightintensity" );
	if (!lightIntensityModifier) lightIntensityModifier = new float( 1.0 );

	extinctionCoef = this->GetSceneFloatVar( "extinctioncoef" );
	if (!extinctionCoef) extinctionCoef = new float( 0.0 );
